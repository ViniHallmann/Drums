...
export class ChartParser {
    ...
    private validate(json: ChartJSON): void {
        if (!json.metadata?.bpm) {
        throw new Error('Chart must have BPM');
        }
        if (!Array.isArray(json.notes)) {
        throw new Error('Chart must have notes array');
        }
        // Mais validações...
    }

    private convertToSeconds(time: number, bpm: number): number {
        // Se time > 100, assume que são ticks
        if (time > 100) {
        const TICKS_PER_BEAT = 256;
        const beats = time / TICKS_PER_BEAT;
        return (beats / bpm) * 60;
        }
        // Se não, assume que já está em segundos
        return time;
    }
